...
SELECT*, last_name,count(*) AS freq FROM sakila.actor
group by last_name
HAVING freq = 1; 
...
SELECT staff_id,count(rental_id) AS rental_num FROM sakila.rental
GROUP BY staff_id;

-- 4.Using the film table, find out how many films were released each year.
SELECT release_year,count(film_id) AS film_num FROM sakila.film
GROUP BY release_year;

-- 5.Using the film table, find out for each rating how many films were there.
SELECT rating,count(film_id) AS film_num FROM sakila.film
GROUP BY rating;
